...
static void HealpixBaseObject_dealloc(HealpixBaseObject* self) {
    self->ob_type->tp_free((PyObject*)self);
}

// Allocate memory for Python object and Healpix_Base (__new__)
static PyObject *HealpixBaseObject_new(PyTypeObject *type, 
        PyObject *args, PyObject *kwds) {
    HealpixBaseObject *self;
    self = (HealpixBaseObject *) type->tp_alloc(type, 0);
    return (PyObject *) self;
}

// Initialize object (__init__)
static int HealpixBaseObject_init(HealpixBaseObject *self, 
        PyObject *args, PyObject *kwds) {
    int nside=-1;
    PyObject *order = NULL;
    ...
    try {
        if (nside == -1) self->healpix_base = Healpix_Base();
        else self->healpix_base = Healpix_Base(nside, scheme, SET_NSIDE);
...
static PyObject * HealpixBaseObject_npix2nside(HealpixBaseObject *self,
        PyObject *args) {
    int npix;
    if (!PyArg_ParseTuple(args, "i", &npix)) return NULL;
    try {
        return PyInt_FromLong(self->healpix_base.npix2nside(npix));
    } catch (Message_error &e) {
        PyErr_Format(PyExc_RuntimeError, e.what());
        return NULL;
    }
 ...
 */
static PyObject * HealpixBaseObject_nest_ring_conv(HealpixBaseObject *self,
        PyObject *args) {
    PyArrayObject *px;
    PyObject *ordering;
    ...
    try {
        if (strcmp(PyString_AsString(ordering), "NEST") == 0) {
            for (int i=0; i < DIM(px,0); i++)
                IND1(px,i,long) = self->healpix_base.ring2nest(IND1(px,i,long));
        } else if (strcmp(PyString_AsString(ordering), "RING") == 0) {
            for (int i=0; i < DIM(px,0); i++)
                IND1(px,i,long) = self->healpix_base.nest2ring(IND1(px,i,long));
        } else {
...
static PyObject * HealpixBaseObject_SetNside(HealpixBaseObject *self,
        PyObject *args) {
    Healpix_Ordering_Scheme scheme = RING;
    int nside;
    ...
    self->healpix_base.SetNside(nside, scheme);
    Py_INCREF(Py_None);
    return Py_None;
}
 ...
 */
static PyObject * HealpixBaseObject_crd2px(HealpixBaseObject *self,
        PyObject *args) {
    ...
    if (DIM(crd,1) == 2) {
        ...
        for (int i=0; i < sz; i++) {
            ...
            if (std::isfinite(p.theta) && std::isfinite(p.phi))
                IND1(rv,i,long) = self->healpix_base.ang2pix(p);
            else {
        ...
        for (int i=0; i < sz; i++) {
            ...
            if (std::isfinite(v.x) && std::isfinite(v.y) && std::isfinite(v.z))
                IND1(rv,i,long) = self->healpix_base.vec2pix(v);
            else {
                printf("Warning: encountered NaN/Inf in crd2px\n");
                IND1(rv,i,long) = 0;
            }
 ...
 */
static PyObject * HealpixBaseObject_px2crd(HealpixBaseObject *self,
        PyObject *args, PyObject *kwds) {
    pointing p;
    vec3 v;
    PyArrayObject *px, *rv;
    ...
    // Interpret coordinates
    for (int i=0; i < sz; i++) {
        p = self->healpix_base.pix2ang(IND1(px,i,int));
        if (return_type == 0) {
...
static PyObject * HealpixBaseObject_Order(HealpixBaseObject *self) {
    return PyInt_FromLong(self->healpix_base.Order());
}

// Thin wrapper over Healpix_Base::Nside
static PyObject * HealpixBaseObject_Nside(HealpixBaseObject *self) {
    return PyInt_FromLong(self->healpix_base.Nside());
}

// Thin wrapper over Healpix_Base::Npix
static PyObject * HealpixBaseObject_Npix(HealpixBaseObject *self) {
    return PyInt_FromLong(self->healpix_base.Npix());
}

// Thin wrapper over Healpix_Base::Scheme
static PyObject * HealpixBaseObject_Scheme(HealpixBaseObject *self) {
    Healpix_Ordering_Scheme scheme = self->healpix_base.Scheme();
 ...
 */
static PyObject * HealpixBaseObject_get_interpol(HealpixBaseObject *self,
        PyObject *args, PyObject *kwds) {
    pointing p;
    vec3 v;
    ...
    static char *options[] = {"RING", "NEST", NULL};
    static char *kwlist[] = {"crd", "ordering", NULL};
    PyObject *order = NULL, *rv = NULL;
    // Parse and check input arguments
    if (!PyArg_ParseTupleAndKeywords(args, kwds,"O!|O", kwlist, 
            &PyArray_Type, &crd, &order))
        return NULL;
    int scheme = get_option(options, order);
    ...
    for (int i=0; i < sz; i++) {
        if (RANK(crd) == 1) {
            p = self->healpix_base.pix2ang(IND1(crd,i,long));
        } else if (DIM(crd,1) == 2) {
            p.theta = IND2(crd,i,0,double);
            p.phi = IND2(crd,i,1,double);
            if (!std::isfinite(p.theta) || !std::isfinite(p.phi)) {
        ...
        if (scheme == 0) self->healpix_base.get_interpol2(p, fix_pix, fix_wgt);
        else self->healpix_base.get_interpol(p, fix_pix, fix_wgt);
        for (int j=0; j < 4; j++) {
...
static PyMethodDef HealpixBaseObject_methods[] = {
    {"npix2nside", (PyCFunction)HealpixBaseObject_npix2nside, METH_VARARGS,
        "npix2nside(npix)\nConvert number of pixels to number of sides."},
    {"nest_ring_conv", (PyCFunction)HealpixBaseObject_nest_ring_conv,
        METH_VARARGS,
    ...
    {"SetNside", (PyCFunction)HealpixBaseObject_SetNside, METH_VARARGS,
        "SetNside(nside, ordering)\nAdjust Nside and ordering scheme ('RING' or 'NEST')."},
    {"crd2px", (PyCFunction)HealpixBaseObject_crd2px, METH_VARARGS,
    ...
    {"px2crd", (PyCFunction)HealpixBaseObject_px2crd,METH_VARARGS|METH_KEYWORDS,
    ...
    {"Order", (PyCFunction)HealpixBaseObject_Order,METH_NOARGS,
        "Order()\nReturn the order parameter."},
    {"Nside", (PyCFunction)HealpixBaseObject_Nside,METH_NOARGS,
        "Nside()\nReturn the Nside parameter."},
    {"Npix", (PyCFunction)HealpixBaseObject_Npix,METH_NOARGS,
        "Npix()\nReturn the number of pixels in the map."},
    {"Scheme", (PyCFunction)HealpixBaseObject_Scheme,METH_NOARGS,
        "Scheme()\nReturn the ordering scheme of the map ('NEST' or 'RING')."},
    {"get_interpol", (PyCFunction)HealpixBaseObject_get_interpol,
...
static PyTypeObject HealpixBaseType = {
    PyObject_HEAD_INIT(NULL)
    0,                         /*ob_size*/
    "_healpix_base.HealpixBase", /*tp_name*/
    sizeof(HealpixBaseObject), /*tp_basicsize*/
    0,                         /*tp_itemsize*/
    (destructor)HealpixBaseObject_dealloc, /*tp_dealloc*/
    0,                         /*tp_print*/
    0,                         /*tp_getattr*/
    0,                         /*tp_setattr*/
    0,                         /*tp_compare*/
    ...
    "Functionality related to the HEALPix pixelisation.  HealpixBase() or HealpixBase(nside, ordering='RING').",       /* tp_doc */
    0,                     /* tp_traverse */
    0,                     /* tp_clear */
    0,                     /* tp_richcompare */
    0,                     /* tp_weaklistoffset */
    0,                     /* tp_iter */
    0,                     /* tp_iternext */
    HealpixBaseObject_methods,             /* tp_methods */
    0,                     /* tp_members */
    0,                         /* tp_getset */
    0,                         /* tp_base */
    0,                         /* tp_dict */
    ...
    0,                         /* tp_descr_set */
    0,                         /* tp_dictoffset */
    (initproc)HealpixBaseObject_init,      /* tp_init */
    0,                         /* tp_alloc */
    HealpixBaseObject_new,       /* tp_new */
};

// Module methods
static PyMethodDef _healpix_base_methods[] = {
    {NULL}  /* Sentinel */
};
...
// Module init
PyMODINIT_FUNC init_healpix_base(void) {
    PyObject* m;
    HealpixBaseType.tp_new = PyType_GenericNew;
    if (PyType_Ready(&HealpixBaseType) < 0) return;
    m = Py_InitModule3("_healpix_base", _healpix_base_methods,
    ...
    import_array();
    Py_INCREF(&HealpixBaseType);
    PyModule_AddObject(m, "HealpixBase", (PyObject *)&HealpixBaseType);
}
