...
def maxent(im, ker, var0=None, mdl=None, gain=.1, tol=1e-3,
    ...
    if var0 is None:
        # Get an estimate of noise by doing lsq and using residual.  MEM will
        # rely on fact that we're spending less time optimizing in lsq, so
        # there should be a little play-room for taking entropy into account.
        x,res = lsq(im,ker,gain=gain,tol=tol,maxiter=maxiter/2,verbose=verbose)
        var0 = numpy.var(res)
    if verbose: print 'Using var0 =', var0
    Nvar0 = d_i.size * var0
    inv_ker = numpy.fft.fft2(ker)
    m_i = mdl.flatten()
