...
class UV(_miriad.UV):
    ...
    def __init__(self, filename, status='old'):
        ...
        if status == 'old':
            ...
            self.raw_read(4096); self.rewind() # Update variables for the user
    ...
    def read(self):
        ...
        return preamble, n.ma.array(data, mask=n.logical_not(flags))
    def all(self):
    ...
    def write(self, preamble, data):
        ...
        flags = n.logical_not(data.mask).astype(n.int32)
        if len(flags.shape) == 0: flags = n.ones(data.shape, dtype=n.int32)
        self.raw_write(preamble, data.data.astype(n.complex64), flags)
    ...
    def pipe(self, uv, mfunc=echo, append2hist=''):
