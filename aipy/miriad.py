...
class UV(_miriad.UV):
    ...
    def write(self, preamble, data, flags=None):
        ...
        if not flags is None: flags = n.logical_not(flags).astype(n.int32)
        elif len(data.mask.shape) == 0:
            flags = n.ones(data.shape, dtype=n.int32)
            data = data.unmask()
        else:
            flags = n.logical_not(data.mask).astype(n.int32)
            #data = data.filled(0)
            data = data.data
        self.raw_write(preamble, data.astype(n.complex64), flags)
