...
class HealpixMap(HealpixBase):
    ...
    def interpolated_add_val(self, crd, data):
        ...
        aipy.utils.add2array(self.map, px, wgts)
    ...
    def from_fits(self, filename, hdunum=1, colnum=0):
        ...
        hdu = aipy.pyfits.open(filename)[hdunum]
    ...
    def to_fits(self, filename, format=None):
        ...
        hdu0 = aipy.pyfits.PrimaryHDU()
        col0 = aipy.pyfits.Column(name='signal', format=format, array=self.map)
        cols = aipy.pyfits.ColDefs([col0])
        tbhdu = aipy.pyfits.new_table(cols)
        self._set_fits_header(tbhdu.header)
        hdulist = aipy.pyfits.HDUList([hdu0, tbhdu])
